                    ...
                    <TableBody className="table-body rounded-md">
                        {tableData?.data?.map((item: any, index: number) => (
                            <TableRow
                                className="table-row"
                                key={item.id}
                                onDoubleClick={() => router.visit(`/admin/resources?selected_page_id=${item.id}`)}
                            >
                                ...
                                <TableCellActions>
                                    {item.deleted_at ? (
                                        <RecoverItem
                                            ...
                                            recoverPath={`/admin/resources/${item.id}/recover`}
                                            permission="user update"
                                        />
                                    ) : (
                                        <>
                                            {/* Edit Dialog */}
                                            <EditItemButton url={`/admin/resources/${item.id}/edit`} permission="resource update" />

                                            {/* View Dialog */}
                                            <ViewItemButton url={`/admin/resources/${item.id}`} permission="resource view" />

                                            {/* Delete Item */}
                                            <DeleteItemButton deletePath="/admin/resources/" id={item.id} permission="resource delete" />
                                        </>
                                    )}
                                </TableCellActions>
                                <TableCell>
